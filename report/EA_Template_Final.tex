...
When initializing an individual legally, one makes sure that the generated path exists. Therefore, a city is chosen at random, after which the next city in the tour is chosen to be a random one out of all the neighbors with a non-infinite path cost. If, however, no existing neighbors are available anymore as next cities, the whole procedure restarts.
...
Therefore, 20 \% % TODO still correct?
...
Self adaptivity, time constraint, multiprocessing
Population size
...
An individual also gets assigned a random $\alpha$ value, which represents the probability that the individual will mutate in the mutation step of the algorithm. % This is self-adaptivity

% TODO population size? Biases introduced? Time constraint on construction? Check code for other discrepancies.
% Multiprocessing
