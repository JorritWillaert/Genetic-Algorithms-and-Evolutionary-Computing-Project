...
def selection(distanceMatrix: np.ndarray, population: List[Individual], k: int, all_fitnesses_hashmap: dict) -> Individual:
...
def fitness_sharing_elimination_k_tournament(distanceMatrix: np.ndarray, population: List[Individual], offsprings: List[Individual], lambd: int, all_distances_hashmap: dict, all_fitnesses_hashmap: dict) -> List[Individual]:
    ...
    distances = np.zeros((len(all_individuals), lambd))
    for i in range(lambd - 1):
        # Best possible approach to reduce computational cost --> Only recalculate fitness for the individuals that need recomputation 
        # (for most of them, their fitness will stay the same)
        fvals = fitness_sharing(distanceMatrix, all_individuals, survivors[0:i-1], distances, fitnesses, all_distances_hashmap)
...
def fitness_sharing(distanceMatrix: np.ndarray, population: List[Individual], survivors: np.ndarray, distances: np.ndarray, original_fits: np.ndarray, all_distances_hashmap: dict) -> np.ndarray:
    if not survivors:
    ...
    shared_part = (1 - (distances[:, 0: len(survivors)] / sigma) ** alpha)
    shared_part *= np.array(distances[:, 0:len(survivors)] <= sigma)
    ...
    def optimize(self, filename: str) -> float:
        ...
        while(True): 
            ...
            for offspring in range(p.num_offsprings):
                parent1 = selection(distanceMatrix, population, p.k, all_fitnesses_hashmap)
                parent2 = selection(distanceMatrix, population, p.k, all_fitnesses_hashmap)
